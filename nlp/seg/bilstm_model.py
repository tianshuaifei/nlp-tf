...
class SegTagger(object):
    def __init__(self, is_training, config):
        ...
        # 通过clip_by_global_norm控制梯度的大小，避免梯度膨胀的问题
...
def _lstm_model(inputs, targets, config):
    ...
    state = initial_state
    with tf.variable_scope("seg_lstm"):
...
def _bilstm_model(inputs, targets, config):
    ...
    # adding extra statistics to monitor
    # 添加额外的统计监测
    print("tf.cast(tf.argmax(logits, 1), tf.int32)")
    print(tf.cast(tf.argmax(logits, 1), tf.int32))
