...
# input format word2_tag2 word2_tag2
...
def load_data(data_config):
 ...
 #   word_to_id, tag_to_id = _build_vocab(train_path)
    if not os.path.isfile(data_config.word_to_id):
        print("根据训练数据构建词典，标签字典，并写入文件")
        word_to_id, tag_to_id = _build_vocab(train_path)
        _save_vocab(word_to_id, data_config.word_to_id)
        _save_vocab(tag_to_id, data_config.tag_to_id)
    else:
        word_to_id = _read_vocab(data_config.word_to_id)
        tag_to_id = _read_vocab(data_config.tag_to_id)
...
def main():
    ...
    """
    #  data_path ="E:/tsf_tf/trunk/zutnlp/pos/data"
    data_path = "data/demo"
    ...
    ids = [1, 4, 2, 7, 20]
    sentence = word_ids_to_sentence(data_path, test_word[:100])
